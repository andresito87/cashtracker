		...
		<Dialog open={isOpen} onClose={handleClose} className="relative z-50">
			...
			<div className="fixed inset-0 z-10 w-screen overflow-y-auto">
				<div className="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
					<DialogPanel
						transition
						className="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all duration-300 ease-out data-closed:translate-y-4 data-closed:opacity-0 data-closed:scale-95 sm:my-8 sm:w-full sm:max-w-lg p-6 sm:p-8 space-y-6"
					>
						<div className="flex items-start gap-4">
							<div className={`p-3 ${iconStyles} rounded-full shrink-0`}>
								<svg
									className="w-6 h-6"
									fill="none"
									viewBox="0 0 24 24"
						...
						<div className="flex flex-row items-center justify-end gap-3 pt-4 border-t border-gray-100">
							<button
								...
								className={`w-full sm:w-auto px-5 py-2.5 rounded-xl ${buttonStyles} text-white font-semibold text-sm shadow-md transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-75 cursor-pointer active:scale-95`}
							>
								{isProcessing ? (
									<>
										<svg
