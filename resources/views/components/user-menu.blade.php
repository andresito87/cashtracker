@auth
	<div class="relative inline-block text-left" id="user-menu-container">
		<button type="button"
		        ...
		        aria-expanded="false"
		        aria-haspopup="true"
		        class="flex items-center gap-2.5 text-sm sm:text-base font-medium text-gray-100 hover:text-white bg-purple-950/60 hover:bg-purple-900/80 border border-purple-700/40 px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-xl transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-purple-500/50 shadow-sm active:scale-95">
			<!-- User Avatar Icon -->
			<span
				class="flex items-center justify-center w-7 h-7 rounded-full bg-purple-600/40 border border-purple-400/30 text-purple-200 shrink-0">
			...
			<!-- User Name -->
			<span class="font-semibold text-white tracking-wide">{{ auth()->user()->name }}</span>

			<!-- Chevron Down Icon -->
			<svg class="w-4 h-4 text-purple-300 transition-transform duration-200" id="user-menu-chevron" fill="none"
		...
		<div id="user-menu-dropdown"
		     class="hidden absolute right-0 mt-2 w-56 rounded-2xl bg-[#1d0d3a] border border-purple-800/60 shadow-2xl shadow-purple-950/90 p-1.5 text-sm text-gray-200 z-50 divide-y transition-all duration-150 origin-top-right">
