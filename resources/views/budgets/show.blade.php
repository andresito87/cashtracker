...
@section('contents')
    <div class="py-10 bg-slate-50/70 min-h-[calc(100vh-80px)]">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 hover:bg-gray-100 font-semibold text-xs uppercase tracking-wider shadow-xs transition-all duration-200 active:scale-95 group">
            ...
            <div class="bg-white border border-purple-900/10 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-gray-100">
                    ...
                    <div>
                        <span class="inline-flex items-center rounded-lg px-3 py-1.5 text-xs font-semibold {{ $budget->type === \App\Enums\BudgetType::Goal ? 'bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20' : 'bg-purple-50 text-purple-700 ring-1 ring-inset ring-purple-700/20' }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div class="bg-gray-50/50 border border-gray-100 rounded-xl p-5">
                        <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-1">{{ __('messages.amount') }}</span>
                        <p class="text-3xl font-extrabold text-gray-900">{{ $budget->formattedAmount() }}</p>
                    </div>

                    <!-- Type Info Box -->
                    <div class="bg-gray-50/50 border border-gray-100 rounded-xl p-5">
                        <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-1">{{ __('messages.type') }}</span>
                ...
                <!-- Description -->
                <div>
                    <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-1.5">{{ __('messages.description') }}</span>
                    <div class="bg-slate-50/80 rounded-xl p-4 border border-gray-100 text-gray-700 text-sm leading-relaxed">
                ...
                <div class="pt-6 flex flex-col sm:flex-row items-center justify-end gap-3 border-t border-gray-100">
                    ...
                    <form method="POST" action="{{ route('budgets.destroy', $budget) }}" class="w-full sm:w-auto" onsubmit="return confirm('{{ __('messages.confirm_delete') }}');">
                        @csrf
                        @method('DELETE')
                        <button
                            type="submit"
